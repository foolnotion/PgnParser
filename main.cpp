...
loses the bishop} 5... bxa6 $16 6. O-O Nxe4 $2 7. Qxe4 $10 *";
    ...
    PgnParser<Iter, Skipper> parser;
    PgnDatabase db;

    Iter iter = pgn_data.begin();
    Iter last = pgn_data.end();
    auto success = qi::phrase_parse(iter, last, parser, qi::space, db);
    if (success && iter == last) {
